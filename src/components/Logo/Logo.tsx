...
export type LogoVariant = "red" | "white" | "black" | "current";

export interface LogoProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  /**
   * Which fill to render. "current" inherits the CSS `color` of its
   * container, useful when the surrounding component already controls
   * light/dark theming. Defaults to "red" (the brand mark's default fill).
   */
  variant?: LogoVariant;
  /** Rendered height in pixels; width follows the mark's fixed aspect ratio (~3.71:1). */
  height?: number;
  /** Accessible label. Omit (pass `""`) if the logo is purely decorative next to visible OCBC text. */
  title?: string;
}

const VARIANT_COLOR: Record<Exclude<LogoVariant, "current">, string> = {
  red: "var(--ocbc-red)",
  white: "var(--ocbc-white)",
  black: "var(--ocbc-charcoal)",
};

/**
 * OCBC wordmark (the junk-ship-in-an-"O" mark adopted 2023). Single-path
 * SVG driven by `currentColor`, so `variant` (or the surrounding CSS
 * `color`) fully controls its appearance, no separate asset files needed
 * for light/dark contexts.
 *
 * Reference-quality vector (traced from a public source, not OCBC's own
 * production master); see the brand-os skill's assets/logo/SOURCE_AND_LICENSE.md
 * for provenance. Treat as a design/dev placeholder until swapped for
 * OCBC's own brand-kit file ahead of anything customer-facing at scale.
 */
export function Logo({ variant = "red", height = 32, title = "OCBC", style, ...rest }: LogoProps) {
  const color = variant === "current" ? "currentColor" : VARIANT_COLOR[variant];
  return (
    <svg
      viewBox="0 0 148.27 40"
      height={height}
      width={height * (148.27 / 40)}
